...
merge(
    ...
    undefined,
    {
        ...
        'Current Network Load shows how much raw bandwidth is currently used to serve images.':
            '当前网络负载显示当前用于提供图像的原始带宽量。',
        ' This includes requests served by H@H as well as direct requests from the image servers.':
            '这包括 H@H 服务的请求以及来自图像服务器的直接请求。',
        ...
        '. Make sure to read the requirements first to make sure that you qualify.':
            '。请务必先阅读要求以确保您符合资格。',
        ' Include the specs for the client in the message, and specify whether it is a home connection or a VPS/Dedicated.':
            '在消息中包含客户端的规格，并指定它是家庭连接还是 VPS/独立服务器。',
        ' Each client requires its own unique public IPv4 address to run, and must either be reachable directly from the Internet, or have a port forwarded.':
            '每个客户端都需要拥有唯一的公共 IPv4 地址才能运行，并且必须可以直接从 Internet 访问，或者使用端口转发。',
        ...
        'If the auto-detected country is wrong, contact Tenboro to have an override applied.':
            '如果自动检测的国家/地区错误，请联系 Tenboro 以进行更改。',
        ' Include the correct country, client ID and IP address in your message, and make sure to keep your client running.':
            '在消息中包含正确的国家/地区、客户端 ID 和 IP 地址，并确保您的客户端保持运行。',
        ...
        ' Clients must have been running for more than 24 hours with a hitrate above 1 to qualify.':
            '客户端必须保持运行超过 24 小时且点击率高于 1 才能获得资格。',
        ...
        File: '文件',
        Size: '大小',
        'Please verify that the size and cryptographic hashes correspond to the file you download.':
            '请验证您下载的文件的大小和哈希值与上方给出的数值相对应。',
        ' For more information about file validation, see these links:':
            '有关文件验证的更多信息，请参阅以下链接（英文）：',
        'Hentai@Home is an Open Source project released under the GNU General Public Licence v3.':
            'Hentai@Home 是在 GNU 通用公共许可证 v3 下发布的开源项目。',
        ' The source code and build scripts for Windows and Linux-like systems can be found above.':
            'Windows 和类 Linux 系统的源代码和构建脚本可以在上面找到。',
    },
    [],
);
