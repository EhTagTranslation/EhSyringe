...
merge(
    ...
    undefined,
    {
        ...
        'Download ': '同时下载 ',
        ' images at the same time (≤ 5 is recommended)': ' 张图片 (建议 ≤5 张)',
        'Abort downloading current image after ': '无响应 ',
        ' second(s) (0 is never abort)': ' 秒后终止当前图片下载 (0 为永不终止)',
        ...
        ' Number images (001：01.jpg, 002：01_theme.jpg, 003：02.jpg...) (Separator ':
            ' 使用数字编号图片 (如 001：01.jpg, 002：01_theme.jpg, 003：02.jpg...) (分隔符 ',
        ' Number images with original page number if pages range is set':
            ' 如果设置了页面范围，则使用原始页码对图片进行编号',
        ' Retry automatically when images download failed': ' 图片下载失败后自动重试',
        ' Get downloaded images automatically when canceled downloading': ' 取消下载时自动获取已下载完成的图片',
        ...
        ' Disable requesting and showing image limits': ' 不显示图片配额',
        ...
        ' Force download resized image (never download original image) ': ' 强制下载重采样图片 (从不下载原图) ',
        ' Never get new image URL when failed to download image ': ' 图片下载失败时，不要获取新的图片地址 ',
        ' Never send "nl" GET parameter when getting new image URL ': ' 获取新图片地址时，不带 "nl" 参数 ',
        ...
        ' Force drop downloaded images data when pausing download': ' 暂停下载时强制中断并删除未下载完的图片',
        ' Save as CBZ (Comic book archive) file': ' 以 CBZ (漫画归档) 文件格式保存 ',
        ' Pass cookies manually when downloading images ': ' 下载图片时手动传递 cookie ',
        ...
        '. Only needed if you have the problem, because the audio-playing icon maybe annoying.\t\t\t\t\t\t':
            '。只有当你遇到相关问题时才需开启，因为音乐播放图标可能会很烦人。',
        ...
        ' is a file type to archive comic images, you can open it with some comic viewer like CDisplay/CDisplayEX, or just extract it as a Zip file. To keep the order of images, you can also enable numbering images.\t\t\t\t\t\t':
            ' 是一种归档漫画图片的文件类型，您可以使用 CDisplay/CDisplayEX 等漫画查看器打开它，或者直接将其视为 Zip 文件解压。为了保持图片的顺序，您还可以启用对图片进行编号。',
        ...
        'eg. -10,12,14-20,27,30-40/2,50-60/3,70-': '如 -10,12,14-20,27,30-40/2,50-60/3,70-',
        ...
        //     '自2019年8月起，1 point 每 0.1 MB，不足 0.1 MB 也计算在内。\n高峰期下载原图将消费GP，GP成本与重置查看限制相同。\n预计GP成本比使用官方存档下载略高，因为每个图片的总和会比打包更大。',
        ...
        //     '需要下载的页面范围，使用半角逗号 (,) 分隔\n例如：\n-10:     下载 1-10 页\n12:      下载 12 页\n14-20:   下载 14-20 页\n27:      下载 27 页\n30-40/2: 在 30-40 范围内每隔 2 页下载 (30, 32, 34, 36, 38, 40)\n50-60/3: 在 50-60 范围内每隔 3 页下载 (50, 53, 56, 59)\n70-:     下载 70-最后一页',

        'You can find GID and token at the address bar like this: exhentai.org/g/[GID]/[Token]/':
            '你可以在地址栏中找到 GID 和 token，如 exhentai.org/g/[GID]/[Token]/',
        ...
        'You can find it at the left of the gallery page.': '你可以在图库左侧找到它',

        'Pause (Downloading images will keep downloading)': '暂停 (下载中的图片将继续下载)',
        'Pause (Downloading images will be aborted)': '暂停 (下载中的图片将中断下载)',
        Resume: '继续',
        'Not download? Click here to download': '还未下载？点此开始',

        "If an error occured and script doesn't work, click ": '如果出现错误导致脚本无法工作，请点击 ',
        here: '此处',
        ' to force get your downloaded images.': ' 强制获取已下载的图片。',
